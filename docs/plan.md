...
$ dcs-bindings render
  ...
  Scanning for installed aircraft with joystick bindings...

  Found 12 aircraft with bindings configured:

    [ ] 1.  A-10C_2           (1 seat)
    [ ] 2.  AH-64D_BLK_II    (2 seats: Pilot, CPG)
    [*] 3.  F-14B             (2 seats: Pilot, RIO)
    [*] 4.  FA-18C_hornet     (1 seat)
    [ ] 5.  F-16C_50          (1 seat)
    [*] 6.  Ka-50_3           (1 seat)
  ...
  Select aircraft to generate images for (comma-separated, or 'all'):
  > 3,4,6
...
- Scans for installed aircraft by examining the saved games input config directory
- Shows multi-seat info for each aircraft
- Allows the user to select which aircraft to generate images for
...
#### 1.2 Aircraft Selection from Installed Modules

The tool can detect which aircraft the user has configured bindings for by scanning:
`<saved_games_dir>/Config/Input/`

Each subdirectory corresponds to an aircraft module. The tool will:
1. **Enumerate all aircraft directories** in the input config path
2. **Filter to those with joystick bindings** (at least one device directory containing a binding file)
3. **Detect seat count** per aircraft (see Section 2 - Multi-seat detection)
4. **Present an interactive selection** during setup or via `dcs-bindings select-aircraft`
5. **Store the selection** in `config.yaml` under an `aircraft` key

Selected aircraft can be changed later by editing the config or re-running `dcs-bindings select-aircraft`.
...
# Which aircraft to generate binding images for
# Use 'all' or list specific module names
aircraft:
  - "FA-18C_hornet"
  - "F-14B"
  - "Ka-50_3"
...
# ─── Aircraft selection ───
# Interactively select which aircraft to generate images for
dcs-bindings select-aircraft --config config.yaml

# ─── Rendering ───
# Generate binding images for all selected aircraft (all seats)
# Normal operation: uses cached button positions (fast)
# If no config exists, triggers the interactive setup wizard automatically
...
- [ ] Interactive aircraft selection (present list, store selection in config)
...
| Interactive first-run wizard | Zero-config start for new users — guided prompts for paths, auto-detects installed aircraft, lets user select which to generate. No documentation reading required to get started |
